...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		auth.jdbcAuthentication().dataSource(dataSource)
				...
				.authoritiesByUsernameQuery(
						"select username as pricipale, role as role from users_roles where username=?")
	...
	protected void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().sessionManagement().sessionCreationPolicy(SessionCreationPolicy.STATELESS).and()
				...
				.antMatchers(HttpMethod.GET, "/clients/**/**", "/compteEpargnes/**", "/compteCourants/**",
				...
				.antMatchers("/virements/**", "/saveVirements/**", "/rechargeTelephones/**", "/saveRecharge/**")
